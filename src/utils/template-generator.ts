...
export class TemplateGenerator {
  ...
  generateTemplate(
    ...
    switch (type) {
    case 'sprint':
      return this.generateSprintTemplate(params);
    case 'docref':
      return this.generateDocrefTemplate(params);
    case 'coderef':
      return this.generateCoderefTemplate(params);
    case 'opinions':
      return this.generateOpinionsTemplate(params);
    default:
   ...
   */
  generateProjectPlanTemplates(): Record<string, string> {
    const now = new Date().toISOString().split('T')[0];
    
    return {
      'PLAN.md': this.generatePlanTemplate({ date: now }),
      'CURRENT.md': this.generateCurrentTemplate({ date: now }),
      'M01_S01.initial_setup.md': this.generateSprintTemplate({
        milestone: 'M01',
        sprint: 'S01',
        title: 'initial_setup',
        date: now,
   ...
   */
  private generateSprintTemplate(params: Record<string, any>): string {
    const {
      milestone = 'M01',
      sprint = 'S01',
      title = 'task_name',
...
sprint: ${sprint}
created_date: ${date}
description: ${title.replace(/_/g, ' ')} execution plan
purpose: Detailed implementation plan for ${title.replace(/_/g, ' ')}
---

# ${milestone}_${sprint}: ${title.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
...
`;
   ...
   */
  private generateDocrefTemplate(params: Record<string, any>): string {
    const {
    ...
    return `---
created_date: ${date}
description: Reference documentation for ${target}
...
# ${target.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())} Reference
...
`;
   ...
   */
  private generateCoderefTemplate(params: Record<string, any>): string {
    const {
    ...
    return `---
created_date: ${date}
description: Code reference for ${target}
...
# ${target.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())} Code Reference
...
`;
   ...
   */
  private generateOpinionsTemplate(params: Record<string, any>): string {
    const {
...
# ${topic.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
...
`;
   ...
   */
  private generatePlanTemplate(params: Record<string, any>): string {
    const { date } = params;
...
`;
   ...
   */
  private generateCurrentTemplate(params: Record<string, any>): string {
    const { date } = params;
