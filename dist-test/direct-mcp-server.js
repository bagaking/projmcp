    ...
    try {
        ...
        const filteredFiles = files.filter(file => {
            if (type === 'all')
                return true;
            if (type === 'sprint')
                return file.match(/^M\d{2}_S\d{2}/);
            if (type === 'doc')
                return file.startsWith('DOCREF_');
            if (type === 'code')
                return file.startsWith('CODEREF_');
            if (type === 'opinion')
                return file.startsWith('OPINIONS_');
            return false;
        });
        return {
            content: [{
...
server.registerTool("show_current", {
